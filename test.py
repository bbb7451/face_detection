...
class SCRFD:
    ...
    def prepare(self, ctx_id=-1, nms_thresh=0.4):
        self.nms_thresh = nms_thresh

    def detect(self, img, thresh=0.5):
        h0, w0, _ = img.shape
        det_img = cv2.resize(img, self.input_size)
        blob = cv2.dnn.blobFromImage(det_img, 1/128, self.input_size, (127.5,127.5,127.5), swapRB=True)
...
if __name__ == '__main__':
    ...
    faces, kpss = detector.detect(img, thresh=0.5)
    if len(faces)==0:
        print("얼굴을 찾지 못했습니다.")
    else:
        ...
        output_path = 'detected_testImage1_kps.jpg'
        cv2.imwrite(output_path,img)
        print(f"얼굴+랜드마크 감지 완료: {output_path}")
